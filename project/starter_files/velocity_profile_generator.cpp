...
double VelocityProfileGenerator::calc_distance(const double& v_i,
  ...
  else
  {
    ...
    d = 0;  // <- Update
   // Using equation of motion for constant acceleration:
   //Reference section 3.28  Motion Planning d = (v_f^2 - v_i^2) / (2 * a)
   // v_f^2 = v_i^2 + 2*a*d
   // Solve for d: d = (v_f^2 - v_i^2) / (2*a)
   d = (double)((v_f * v_f - v_i * v_i) / (2.0 * a)); //cast result to double
...
double VelocityProfileGenerator::calc_final_speed(const double& v_i,
  ...
  // Optimized calculation of final velocity
  v_f = std::sqrt(v_i * v_i + 2 * d * a);

  // Optimized calculation of discriminant
  disc =(double) ((v_f * v_f) - (4 * a * v_i));
